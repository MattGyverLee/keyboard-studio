...
Counts recomputed directly from `criteria.json` array length and per-band occurrence counts. 133 of the 146 are the original repo-hygiene criteria, accounting for all 133 colored criterion spans in `docs/criteria.md` (the 136-span count cited in earlier passes included the 3 legend-definition spans at lines 6-8, which are not criteria; see "Omitted rows" below). The remaining 13 are: 12 **section-18 "Design heuristics (DISCUS)"** rows added to operationalize the keyboard-design principles in [docs/keyboard-design-principles.md](../../../docs/keyboard-design-principles.md) — 7 `layer-c-enforce` (auto-checkable on the first-draft touch layout / output coverage), 3 `yellow-survey` (judgement-dependent), and 2 `red-checklist` (inherently human) — plus 1 **section-19 import output** criterion (`layer-c-enforce`). See [docs/discus-principles-integration.md](../../../docs/discus-principles-integration.md) for the full DISCUS → programmability mapping.
