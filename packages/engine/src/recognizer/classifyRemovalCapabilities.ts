 ...
 *   outputStore.nodeId  -> "removable:slot-fill"   (alias for S-02 body rules,
 *                          so slot tiles keyed by "<outputStoreNodeId>#<i>"
 *                          can resolve by stripping the "#<i>" suffix)
 *
 ...
 * whenever isBody(rule) is true, regardless of whether the body's label came from
 * ownership (step 2) or the isBody shape (step 3). This keeps the alias contract
 * intact for IRs where recognizePatterns() was not applied.
 *
...
export function classifyRemovalCapabilities(ir: KeyboardIR): Map<string, RemovalCapability> {
  ...
  for (const group of ir.groups) {
    for (const rule of group.rules) {
      // Compute isBody once — used both to emit the output-store alias and as
      // a fallback label in step 3. Independent of which branch assigns the label.
      const bodyShape = isBody(rule);

      // Alias: emit outputStore.nodeId -> removable:slot-fill whenever the rule
      // has the S-02 body shape, regardless of which branch below labels the rule.
      if (bodyShape) {
      ...
      // and owned cluster members already handled above)
