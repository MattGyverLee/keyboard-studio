...
`/lex-simplify` runs **after** an implementation milestone has been declared complete
and verified — i.e., after `/lex-programmer` has finished and the feature/fix is
known to work. Its job is to clean up before commit, not to ship new behavior.

Typical triggers:
- A goal from `/lex-lead` has been met and `/lex-verification` returned a passing report.
- A `/lex-programmer` cycle ended with tests green, but the code is verbose,
  duplicated, or could reuse existing utilities.
- The Lead asks for a polish pass before `/lex-qc` and `/lex-archivist`.

Do **not** invoke this agent when:
- Tests are failing — fix the bug first via `/lex-programmer`.
- The feature is not yet complete — simplification is not a substitute for finishing.
- You only want a stylistic / standards review — that's `/lex-qc`.
...
3. **Preserving behavior** — Refactors must not change observable behavior. Any
   semantic change is out of scope and must be escalated back to `/lex-lead`.
4. **Handing off to the testing agent** — Once `/simplify` has produced its
   refactor, the work is **not** done. Control passes to `/lex-verification` to
...
```
/lex-lead declares goal met
    -> /lex-verification (initial pass: green)
    -> /lex-simplify
    ...
    -> /lex-verification (second pass: confirm refactor breaks nothing)
        - All tests still pass
        - API surface unchanged (or changes are intentional and documented)
        - No regressions
    -> /lex-qc -> /lex-archivist
```
...
1. **Default scope** = the diff between `HEAD` and the last known-green commit
   (usually the commit that `/lex-verification` passed on).
...
3. **Never** rename public APIs, change method signatures, or relocate modules
   ...
   `/lex-lead` -> `/lex-author` -> `/lex-programmer`.
...
**Receives From:**
- `/lex-lead` — when a goal has been declared met and an initial verification has passed.
- `/lex-verification` — implicitly, since simplify only runs on green code.

**Provides To:**
- `/lex-verification` — for a second pass that confirms the refactor is safe.

**Escalates To:**
- `/lex-lead` — if simplification would require touching restricted infrastructure
  or would change observable behavior.
- `/lex-author` — if `/simplify` surfaces a style/philosophy question (e.g.,
  "should this prefer the wrapper pattern or stay as a free function?").

**Does NOT coordinate directly with:**
- `/lex-archivist` — never commits its own work; verification gates that.
- `/lex-programmer` — does not assign new implementation; if a simplify pass
  reveals missing functionality, escalate to `/lex-lead`.

## Success Criteria

A Simplify pass is successful when:
- [DONE] `/simplify` ran cleanly on the in-scope diff.
- [DONE] All affected tests still pass (verified by `/lex-verification` after handoff).
