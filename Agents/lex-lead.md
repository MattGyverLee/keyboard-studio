---
name: lex-lead
description: Plans LEX crew review cycles and emits dispatch plans for the main session to execute. Use for review cycles, multi-agent refactors, coordinated crew work. Cannot spawn peers — emits a dispatch_plan block; the main session fans it out.
...
groups:
  - mode: parallel | sequential
    tasks:
      - subagent_type: <lex-programmer | lex-qc | lex-domain | lex-author | lex-verification | lex-synthesis | lex-archivist | lex-doc | lex-simplify | Explore>
...
on_return: <what the main session should do with results — usually "re-invoke /lex-lead with all reports">
```
````
...
on_return: re-invoke /lex-lead with both reports for synthesis
```
````
...
- **Blockers found?** Dispatch `lex-programmer` for fixes, then re-verify.
...
Cycle 1: `lex-programmer` → (await) → Cycle 2: `lex-verification` + `lex-qc` parallel → (await) → final approval.

### Critical bug fix
Cycle 1: `lex-programmer` + `lex-verification` sequential → (await) → final approval.

### Major refactoring
Cycle 1: `lex-programmer` → Cycle 2: `lex-verification` → Cycle 3: `lex-qc` + `lex-domain` + `lex-author` parallel → Cycle 4: `lex-synthesis` → final approval.

### API design
Cycle 1: `lex-domain` + `lex-author` parallel → (await) → Cycle 2: `lex-programmer` → (await) → final approval.
