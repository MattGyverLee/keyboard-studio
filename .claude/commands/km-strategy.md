...
Owns the spec §7 strategy framework — the seven discovery axes (A1–A7), the decision tree (§7.2), the S-01..S-13 strategy catalog, the §7.5 self-check validation table, and the `Pattern.strategyId` / `combinesWith` linkage. You validate that strategy selection, pattern wiring, and the §7.5 table stay mutually consistent.
...
- **Strategy catalog** — confirm the selected strategy (S-01..S-13) is appropriate and its description matches implementation.
