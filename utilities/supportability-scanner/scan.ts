...
import { parseKps } from "../../packages/engine/src/base-browser/kps-parser.ts";
import type { CorpusKeyboardEntry, KeyboardIR } from "@keyboard-studio/contracts";
import { buildProducedSet, ImportStatus } from "@keyboard-studio/contracts";
...
interface ScanReport {
  ...
  /** base-matching index fields (artifact A — keyboard-corpus-index.json). */
  script: string;
  languages: string[];
...
/**
 * Sibling `.kps` for a `.kmn` source — conventionally `source/<id>.kps` next to
 * the `.kmn`. Returns `{ script, languages }`, defaulting to a Latin script with
 * no declared languages when the file is absent or unparseable.
 */
function readKpsMeta(kmnPath: string, keyboardId: string): {
  script: string;
  languages: string[];
} {
  const kpsPath = join(dirname(kmnPath), `${keyboardId}.kps`);
  if (!existsSync(kpsPath)) return { script: "Latn", languages: [] };
  try {
    const meta = parseKps(readFileSync(kpsPath, "utf8"));
    return { script: meta.script, languages: meta.languages };
  } catch {
    return { script: "Latn", languages: [] };
  }
}
...
function scanOne(kmnPath: string, releaseDir: string): ScanReport {
  ...
  const kps = readKpsMeta(kmnPath, keyboardId);
  ...
  // Produced-glyph set for character-overlap scoring (artifact A). Sorted by
  // codepoint for stable diffs.
  base.producedGlyphs = [...buildProducedSet(ir)].sort(
...
/**
 * Build artifact A — the base-matching corpus index (spec §8 step 1). One
 * {@link CorpusKeyboardEntry} per scanned keyboard: declared languages + script
 * (from the `.kps`), the produced-glyph set, and import fidelity. Authoring-time
 * only; never serialized into a keyboard. Deterministic (no timestamp) so a
 * future `--check` can diff it.
 */
function buildCorpusIndex(sorted: ScanReport[]): string {
  const entries: CorpusKeyboardEntry[] = sorted.map((r) => {
    const opaqueFeatureCount = r.opaqueFeatureInventory.reduce(
      (s, o) => s + o.count,
      0,
    );
    // Folder id under release/ (the .kpj/source folder), from the source path
    // `<id>/source/<id>.kmn`.
    const path = `release/${r.source.replace(/\/source\/[^/]+\.kmn$/i, "")}`;
    return {
      id: r.keyboardId,
      path,
      script: r.script,
      languages: r.languages,
      producedGlyphs: r.producedGlyphs,
      importStatus: r.status,
      opaqueFeatureCount,
    };
  });
  const payload = {
    schema: "keyboard-corpus-index/v1",
    note:
      "Generated by utilities/supportability-scanner. Authoring-time base-" +
      "matching index (spec §8 step 1); never written into a keyboard artifact.",
    keyboards: entries,
  };
  return JSON.stringify(payload, null, 2) + "\n";
}
...
function main(): void {
  ...
  if (args.check) {
    ...
    console.error(`[OK] import-corpus.json + keyboard-corpus-index.json are up to date (${reports.length} keyboards).`);
    return;
  }
  ...
  writeFileSync(corpusIndexPath, corpusIndex, "utf8");
