...
**Save & resume (v1.4.0).** The working copy *may* be persisted as a **session snapshot** between instantiation and output, so an author can leave and return to in-progress work. This is a non-destructive snapshot, not a second edit path: a restored working copy is identical to the one serialized, the snapshot is never a route to delivery, and **output (step 15) remains the only way to produce a `.zip` or PR**.

The studio supports two persistence modes:

- **Guest** (the default) — local-only snapshot store, no identity, no network. Full authoring and both delivery paths are available; authoring is never gated behind sign-in. The studio surfaces an ambient, non-modal affordance ("log in or create an account to save your progress") offering sign-in for durability and cross-device resume; declining it never blocks authoring or delivery.
- **Signed-in** — local store *plus* an account-keyed backing store for cross-device resume (§16). Two identity providers are supported and treated as equal first-class routes: **GitHub SSO** (reuses the OAuth backend the PR delivery path already needs, with login-only scopes; the PR-delivery scope is requested incrementally only when the author attempts a PR delivery) and **email + password** (so the studio is fully usable without a GitHub account). Signing in migrates the guest's local projects to the account; the local store remains as an offline cache, so authoring continues to work without network. Signing out stops syncing but does not delete local projects.

A snapshot is identified by a stable `projectId`; either store may hold zero or more snapshots. The snapshot format is plain JSON, with no browser-specific or host-specific references, so the same format works against IndexedDB, a server datastore, or (post-v1) a host filesystem in a desktop build. Save & resume always degrades to the local store when no backing service is configured or reachable, and **never requires a GitHub account** (the ZIP delivery path stays accountless).

**Rehydrate equivalence (Layer A''-style check).** A working copy rehydrated from a snapshot must emit byte-identically to the working copy that produced the snapshot. This is a named, testable check parallel to the Layer A' import-fidelity I-checks (§10); failure to round-trip is a snapshot-format defect, not a user error.

**Failure modes.** When the backing store is unreachable, the studio continues in guest mode against the local cache and surfaces the degradation. A corrupt snapshot is quarantined (kept for diagnosis, never silently overwritten) and the author is offered a clean session. When two tabs of the same `projectId` race, last-write-wins on `updatedAt` with a visible warning in the older tab. When the base keyboard cited by a Track 1 working copy has moved upstream since the snapshot was taken, the snapshot remains authoritative and the base-drift is surfaced as a §10 advisory, not an error.
...
- **Hosting and deployment** — infrastructure is left to the operator; this project ships a static SPA. **(v1.4.0 exception, PROPOSED):** an optional, minimal backing service is permitted solely for the two responsibilities below, and nothing else:
  1. The OAuth token exchange the in-scope GitHub fork+PR delivery path already requires (the client secret cannot live in client code).
  2. Account-keyed save & resume (§12) — identity (GitHub SSO and/or email+password) plus a per-account snapshot store keyed by `projectId`.

  The service stores only what those two responsibilities require: an opaque session record per signed-in user and the snapshot blobs they own. It does not host keyboard delivery, run validation, host shared workspaces, or store any data on behalf of guest users. The service is never required to author, validate, or deliver a keyboard — guest mode degrades to local-only persistence without it, and **no GitHub account is ever required** (the ZIP-download path stays accountless). General-purpose hosting, multi-user collaboration, team workspaces, sharing, and history are out of scope and the operator's responsibility. A desktop (Electron) distribution that fulfils the same responsibilities against the host filesystem instead of a server is a post-v1 candidate covered by this same exception.
