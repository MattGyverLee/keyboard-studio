...
- **Track 2 — adapt an existing keyboard (load and edit):** `instantiateFromExisting` loads an existing keyboard (any `release/` source or uploaded `.kmn`), parses it to IR via the §5a codec, and makes the working copy **that IR with identity preserved** — keyboard ID and existing metadata are retained; version is bumped at output. The session enters via the source-picker head UI (§8 step 1 source selection), skipping the identity-lite prompt because the identity is already known. This is the v1.1.0 single-source-adaptation path.
...
**OSK reflects identity as a visible mutation.** The OSK renders the keyboard name and language on the spacebar caption. Identity edits (keyboard name, BCP47 tag) therefore produce visible OSK mutations — specifically, the spacebar caption changes. Script, base-keyboard, carve, and mechanism (gallery) edits change key glyphs. This means the OSK is a live observable of the working copy's full state, not just its rule output.
...
**OSK spacebar caption as a visible identity mutation.** The OSK renders the keyboard name and language on the spacebar caption (the `KeyboardIdentity` fields `displayName` and `bcp47`). Because the working copy is the live OSK target, identity edits are immediately visible in the OSK as spacebar caption changes. Script, base-keyboard, carve, and mechanism edits change the key glyphs. The OSK is therefore a complete observable of the working copy's current state.
