...
> **Status note (2026-06-30):** `track` and `project_name` are now modular question flows (branch `km/decompose-wizard-questions`, PR pending). Three Phase-G question modules (`track_choice`, `project_display_name`, `project_keyboard_id`) and runner wrappers (`PhaseTrack` / `PhaseProjectName`) replaced the bespoke panels; thin `content/flows/track.modular.yaml` and `content/flows/project_name.modular.yaml` wire the CYOA graph. Both steps render as blown-up drill-down graphs on the developer Flow Map, mirroring Phase A/B/F. The copy/adapt fork remains at the manifest level (`project_name` is `spine:false`, `joinTarget:characters`); `qu-mutate-track` (spec #10) makes the YAML `next` rule the live routing path. Gallery decomposition (carve / mechanisms / touch) remains gated on the loop primitive (spec #9, Matt's call) — see §3.1 and the explicit non-goals in [specs/023-qu-decompose-wizard/spec.md](../../specs/023-qu-decompose-wizard/spec.md).
