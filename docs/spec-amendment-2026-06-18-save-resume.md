...
- **Two identity providers, both first-class.** GitHub SSO is the easy path (it reuses the OAuth backend the PR-delivery path already needs, with login-only scopes; the PR-delivery scope is requested incrementally only at delivery time). Email + password is the floor that keeps "no GitHub account ever required" honest for users who want cross-device resume but don't want a GitHub account.
...
2. **Signed-in mode: optional resume.** Lightweight login backed by a minimal Vercel serverless function + managed datastore. Two identity providers, both first-class:
