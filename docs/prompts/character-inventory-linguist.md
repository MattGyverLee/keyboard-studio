...
When the Phase A result includes a `provenance_orthography_url`, the engine appends a grounding line to the rendered prompt after the `Target Language` line. Do not add trailing prose to the template body — it will conflict with this engine-controlled append.
