...
A maintainer wants reachability defined **per-graph** — `findUnreachable` for manifest editor-steps, `resolveNext` over `next` / `FlowGotoRule[]` for survey questions — so that the guardrail correctly handles `FlowGotoRule` branching that `findUnreachable` is blind to. A boundary-crossing step (e.g. `pb_build_list` as a build-list branch) is checked in the **question** graph.

**Why this priority**: `findUnreachable` (`completeness.ts:475-499`) only knows the manifest's spine / `joinTarget` rule — it has **no awareness** of `FlowGotoRule` branching. Applying it to survey questions would mis-classify branch reachability (e.g. the `pb_build_list` build-list branch reached via the IntroChooser gate). Defining reachability per-graph and running BOTH computations is what makes the bijection correct rather than approximate.

**Independent Test**: For manifest editor-steps, compute reachability via `findUnreachable` (spine-or-transitive-`joinTarget`); for survey questions, compute reachability via `resolveNext` over `next` / `FlowGotoRule[]` (the `buildGraphFromQuestions` edge set, `buildStepGraph.ts:84-112`). Confirm both run and contribute to the union; confirm `pb_build_list` is verified in the question graph (reached as the build-list branch), not the manifest graph.
...
4. **Given** `pb_build_list` (the build-list branch reached via the mandatory IntroChooser discovery-method gate), **When** the guardrail checks its reachability, **Then** it is verified in the **question** graph (via `resolveNext`), confirming the boundary-crossing step is covered.
...
- **A boundary-crossing step (`pb_build_list`)**: checked in the question graph (US3 AC-4); reached as the build-list branch behind the IntroChooser gate, so it must have a rendered drill-down node.
...
- **FR-008**: The guardrail MUST verify `pb_build_list` (the build-list branch reached via the mandatory IntroChooser discovery-method gate) in the **question** graph (via `resolveNext`), confirming the boundary-crossing step is covered there and not in the manifest graph.
...
- **`pb_build_list`**: the BuildListView build-list branch (`survey/PhaseB.tsx:535`, used ~`692`), reached via the mandatory IntroChooser discovery-method gate (~`744`); a registry-keyed drill-down under the opaque `characters` node; the boundary-crossing step checked in the question graph.
...
- **SC-005**: `pb_build_list` (build-list branch) is verified in the **question** graph (via `resolveNext`), confirming the boundary-crossing step is covered.
