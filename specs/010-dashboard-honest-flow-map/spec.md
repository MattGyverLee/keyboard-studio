...
- **FR-006**: A stub node MUST present only information P0 actually possesses (a title / stage identity) and MUST NOT fabricate inputs, writes, completeness, or precise spine ordering it cannot yet derive (those arrive in later phases). FR-006 remains satisfied by [spec-021](../021-qu-wire-galleries/spec.md) FR-004: that feature renders only spec-017-declared `inputs`/`writes` on the gallery/wizard nodes — declared data is not fabricated metadata, so FR-006 is not violated.
