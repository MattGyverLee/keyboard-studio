...
**Decision**: Convert **(a)** the carve/add shell — carve remove-mode (`editors/carve/CarveGallery.tsx`) + the add galleries (`editors/assignLoop/`), retiring the direct `workingCopyStore` carve mutators (`deleteNode`/`restoreNode`/`deleteItem`/`restoreItem`/`restoreAll`/`keepAll`) and the add-gallery's direct selected-pattern IR writes — and **(b)** all **8 strategy-bearing question modules** (the non-empty-`writes` set). Display-only (empty `writes`) stays no-op; answer-store-only / identity-metadata modules are out of scope.
...
**Audit note (must reconcile at gate — G-II).** The spec states the in-scope set is **8 strategy-bearing modules with non-empty `writes`**. A current-tree audit (`packages/studio/src/survey/questions/`, ripgrep `writes:\s*\[\s*irPath`) finds **5** modules presently declaring non-empty `writes`: `a/iso_code.ts`, `a/primary_script.ts`, `a/language_name_english.ts`, `a/pa_copyright_holder.ts`, `b/pb_standard_letters.ts`. The 8-module figure is the spec's stated P5 scope (the §6/P2 strategy-bearing set); the present 5 are the modules whose `writes` are already populated. The gap is expected for a design-only spec written ahead of the contract — **the exact in-scope module set MUST be reconciled against the ratified P2 `writes` declarations when #5b/#232 lands** (G-II). `tasks.md` enumerates per-module conversion tasks for the declared set and flags the reconciliation as a gate task.
