...
- **`@keyboard-studio/contracts`** — the locked Day-1 shared contract: TS types, the seven service interfaces + mocks, fixtures, the criteria catalog at `packages/contracts/data/criteria.json` (148 rows — Day-1 catalog was 145: 133 repo-hygiene + 12 DISCUS design-heuristics; became 146 after the 7.7a split (#120) and 148 after two §19 import-output rows (19.1, 19.2); enforced count (148; 40/66/32/10); see spec §11 and [docs/discus-principles-integration.md](docs/discus-principles-integration.md)), and the runtime **zod schemas** (`src/schemas.ts`) that mirror the locked `Pattern`/`Criterion` types. The dependency root — everything else builds to it.
...
**Relationship between `spec.md` and `strategies.md` (resolved — merged).** The two documents have been unified: the `.kmn` strategy framework (seven discovery axes A1–A7, the decision tree, the S-01…S-12 strategy catalog, building blocks, and the validation table) now lives in **`spec.md` Section 7 (Strategy selection)**. It is wired into the rest of the spec: the survey computes the axes (§7.1), the strategy selector runs the decision tree (§7.2) to pick a strategy, and each `Pattern` (§5) links to its strategy card via the (ratified) optional `strategyId` / `combinesWith` fields. The §7.5 validation table is a self-consistency regression suite — the IPA gap was closed in v1.1.1 by rule 3a (the §7.5 IPA row now tests axis logic; the `sil_ipa` keyboard is MML and out of scope for new authoring); the EuroLatin row was reclassified 2026-06-15 as an MML out-of-scope exemplar, not a tree gap; keep §7.1/§7.2/§7.3 and that table mutually consistent across any edit.

The spec embeds external docs by reference (Sec 19): `docs/KM-Questionnaire.md`, `docs/lint.md`, `docs/criteria.md`, `docs/making-a-template.md`. The latter three (`docs/lint.md`, `docs/criteria.md`, `docs/making-a-template.md`) are present in this working copy. `docs/KM-Questionnaire.md` is not yet in the working copy — fetch from `https://github.com/keyboard-studio/keyboard-studio` if needed.
...
- **Validator layering (spec §10).** Three layers — Layer A validity + Layer B style + Layer C hygiene; Layer A is 9 TS-portable checks + 5 WASM-only (spec §10 has the per-check `kmcmplib` source references). **In code:** Layer A/B — plus the Layer A' import-fidelity checks I1–I5 (`engine/src/validator/layer-a-prime.ts`, `index-import-fidelity.ts`) — live in `engine/src/validator`; Layer C is `@keymanapp/keyboard-lint`. (The spec's `@keymanapp/kmn-validator` package has not been extracted yet.)
...
- `km-strategy` — owns spec §7 strategy framework (A1–A7, decision tree, S-01..S-13, §7.5)
...
**New features still get their own `specs/NNN-<slug>/`** with a creation-order `NNN`, and **cite the governing `spec.md §X` (or its extracted folder)** rather than re-deriving scope. The mirror-numbering convention only applies to spec sections being extracted — new features pick the next free `NNN` above the extracted-section range. Exception: two `NNN` values are intentionally shared by distinct feature specs — `010` (`specs/010-dashboard-honest-flow-map/` and `specs/010-irpath-inputs-writes/`) and `012` (`specs/012-modular-loader-cutover/` and `specs/012-step-model-manifest/`); these were approved as a deliberate grouping.
